...
suppressMessages(library(FactoMineR, warn.conflicts=F , quietly = T))
suppressMessages(library("corrplot", warn.conflicts=F , quietly = T))
suppressMessages(library(nnet, warn.conflicts=F , quietly = T))
suppressMessages(library(plyr, warn.conflicts=F , quietly = T))
suppressMessages(library(cowplot, warn.conflicts=F , quietly = T))
...
suppressWarnings(while(C > 0 & iter < 6){
  iter <- iter + 1
  # #> Regresseurs : [1] "BATI_PERIODE" "ESTOC" "Revenu_Insee_quintile" "EHST" "RP_TAILLE_UU" "is_elec" "MI"                  
...
#traçons le diagramme de Cattel. Un coude dans le diagramme de Cattel indique le nombre de
#variables qu'il faut choisir pour la régression.
...
estm_dpe_acp <- multinom(DPEb ~ BATI_PERIODE + ESTOC + Revenu_Insee_quintile + EHST + 
                         RP_TAILLE_UU + is_elec + MI, data = data_regresseurs2, Hess=T)


# Clean -------------------------------------------------
suppressWarnings(rm(data_reg_ind_sup, data_regresseurs, data_regresseurs2, 
                    dpe_pca,dpe_pca2, ind_sup, list_exclus, Phebus,Phebus_bis, 
                    borne_inf, borne_sup, C, dim, init, iter, m, Regresseurs,Variables))
gc()
