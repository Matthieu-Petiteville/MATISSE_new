...
if(scenario_classement == "ssrec"){
  suppressMessages(suppressWarnings(output_macro <- read_excel(path = MatisseFiles$output_macro_code_iter_0_ssrec_xl, sheet = scenario, skip = 1)))
}else{
  suppressMessages(suppressWarnings(output_macro <- read_excel(path = MatisseFiles$output_macro_code_iter_0_xl, sheet = scenario, skip = 1)))
}
#EMS
...
suppressWarnings(rm(scen, ThreeME, output_macro, IMACLIM, FC, EMS, menage_forme, 
                    S, var_fc_vec))
gc()
